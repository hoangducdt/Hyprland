...
try:
    with open('$STATE_FILE', 'r') as f:
        state = json.load(f)
    state['compositor'] = '$choice'
    with open('$STATE_FILE', 'w') as f:
        json.dump(state, f, indent=2)
except Exception as e:
    print(f'Warning: Could not save compositor choice: {e}')
" 2>/dev/null || true
}
...
║                                                        ▄███                                    ║
║                                                 ▄████████▀                                     ║
║   Niri + DMS Installer - Optimized For CachyOS                                                 ║
...
setup_meta_packages() {
    ...
    local meta_pkgs=(
        ...
        "libcava"						# Cava as shared library for waybar
...
setup_hyprland() {
    ...
    log "✓ Hyprland installed and configured"
}
...
setup_caelestia() {
    ...
    local caelestia_pkgs=(
        "caelestia-cli"                 # Caelestia CLI tools
    )

    install_packages "${caelestia_pkgs[@]}"
...
║            ╚══╝╚══╝ ╚═╝  ╚═╝   ╚═╝   ╚══════╝╚═╝  ╚═╝╚═╝  ╚═══╝╚═════╝      ║
║                                                                               ║
║                    COMPOSITOR INSTALLATION WIZARD                            ║
║                                                                               ║
╚═══════════════════════════════════════════════════════════════════════════════╝
EOF
    echo -e "${NC}"
    ...
    echo -e "${BLUE}═══════════════════════════════════════════════════════════════════════════════${NC}"
    echo -e "${YELLOW}   Select your Wayland Compositor:${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════════════════════════════════${NC}"
    echo ""
...
get_compositor_choice() {
    ...
    save_compositor_choice "$COMPOSITOR_CHOICE"
