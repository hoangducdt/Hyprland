...
EOF
    ...
    mkdir -p "$HOME/.config/onedrive/logs"
...
#SDDM_CONF
    ...
    # Create OneDrive config file
    cat > "$HOME/.config/onedrive/config" <<'ONEDRIVE_CONFIG'
# OneDrive Configuration
#1. Xác thực OneDrive:
#   onedrive
#   (Mở link trong browser, đăng nhập Microsoft, copy URL redirect về)

#2. Kiểm tra folder sẽ sync (DRY RUN - không tải thật):
#   onedrive --synchronize --dry-run

#3. Liệt kê tất cả folder trong OneDrive:
#   onedrive --list-folders
#"


#5. Sync thủ công lần đầu:
...
skip_file = "~*|.~*|*.tmp|*.swp|*.partial|*.temp|desktop.ini|Thumbs.db"
